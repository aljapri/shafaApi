...
export class CommentFetchContext {
  private strategy: IFeedbackFetchStrategy;
  constructor(strategy: IFeedbackFetchStrategy) {
    this.strategy = strategy;
  }
  public async handle(req: Request | any, res: Response, next: NextFunction): Promise<void> {
    try {
      const feedback = await this.strategy.fetch(req);
      if (!feedback || feedback.length === 0) {
        return next(HttpResponse.NotFound('There are no appointment.'));
      }

      const data = HttpResponse.Ok(feedback);
